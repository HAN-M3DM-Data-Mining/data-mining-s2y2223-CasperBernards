...
rawDF = pd.read.csv ('https://raw.githubusercontent.com/HAN-M3DM-Data-Mining/assignments/master/datasets/NB-fakenews.csv')
rawDF
...
cleanDF2 = cleanDF.assign(category=cleanDF['type'].map(label_map))
...
catType = CategoricalDtype(categories=["reliable", "unrrliable"], ordered=False)
...
unreliableText = ' '.join([str(Text) for Text in cleanDF2[cleanDF2['catagory']=='unreliable']['text'].astype(str)])
...
xTrain, xTest, yTrain, yTest = train_test_split(wordsDF, rawDF_label)
